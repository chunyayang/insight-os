...
- **Display scope — functional currency off Analytics.** The currency selector exists only on Analytics (cross-market normalization). Elsewhere (Customers, Campaigns, Dashboard KPI cards, etc.) each monetary record is displayed in its **functional currency** — the client reads that record's functional-currency key from the `Money` map. Every monetary resource therefore declares a **`functionalCurrency: CurrencyCode`** so the frontend knows which key to show without any selector. For records whose market is **`All`**, `functionalCurrency` is **`USD`** (international/base currency). The same `Money` map thus serves both the Analytics-normalized view and functional-currency views with no extra request.
