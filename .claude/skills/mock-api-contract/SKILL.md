...
- **The contract obligation:** every monetary resource declares a **`nativeCurrency: CurrencyCode`** naming that record's own currency, so the client can render a local figure with no selector and no extra request. It is a declared field, **not** a function of the record's market — a record whose market is `All` carries whatever currency it declares, and the client must never map market → currency itself (issue #42). One `Money` map serves the Analytics-normalized view, local views, and dual-currency cells alike. **Which of those a given surface renders is decided in `/product-spec` → `currency-model.md`, not here** — that document also owns the pending `nativeCurrency` → `localCurrency` rename (issue #43). Note the client must read this field off the record rather than deriving a currency from the record's market.
