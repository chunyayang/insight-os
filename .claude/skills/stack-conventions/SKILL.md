...
- **Offer a monetary sort control only while the list is narrowed to one market.** A currency-blind ranking across markets is noise (a JPY total always outranks a EUR one), so with the market filter on `All` the sort control renders but stays inert, with a tooltip saying that filtering to a single market enables the ranking — the *disabled + tooltip* treatment, applied here to a meaningfulness rule rather than a permission. Shown rather than hidden so the filter/ranking relationship is discoverable. The endpoint stays capable of sorting the raw field either way; this is a UI affordance, so it is *not* something to re-enforce server-side.
  **Not yet implemented** — `CustomerList.vue` currently offers the LTV sort unconditionally. The rule is specified (product spec §4.5 / §4.6) and scheduled after the ECharts migration — tracked in issue #40. Build any *new* monetary table (Campaigns) to the rule.
